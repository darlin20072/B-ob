...
function toggleAuthMode() {
    ...
    const chooseAvatarTxt =
        document.getElementById('txt-choose-avatar');
    ...
    if (avatarSelector)
        avatarSelector.style.display =
            isRegisterMode ? 'flex' : 'none';

    if (chooseAvatarTxt)
        chooseAvatarTxt.style.display =
            isRegisterMode ? 'block' : 'none';
...
async function handleAuth() {
    ...
    try {
        ...
        console.error(
            "Error detallado en autenticación/Firestore:",
            error
        );
        ...
        .onSnapshot(doc => {
            ...
            if (doc.exists) {
                ...
                if (currentExternalProfileUser) {
                    ...
                    viewUserProfile(
                        currentExternalProfileUser.uid
                    );
            ...
            console.error(
                "Error al leer datos del usuario en tiempo real:",
                error
            );
...
async function viewUserProfile(uid) {
    ...
    if (
        ...
        switchSection(
            'profile',
            document.querySelectorAll('.dock-btn')[2]
        );
    ...
    try {
        ...
        if (bioElem)
            bioElem.innerText =
                currentExternalProfileUser.bio ||
                "Sin biografía configurada.";


        if (statFollowers)
            statFollowers.innerText =
                currentExternalProfileUser.followers
                    ? currentExternalProfileUser.followers.length
                    : 0;


        if (statFollowing)
            statFollowing.innerText =
                currentExternalProfileUser.following
                    ? currentExternalProfileUser.following.length
                    : 0;
...
function goBackToFeed() {
    ...
    switchSection(
        'feed',
        document.querySelectorAll('.dock-btn')[0]
    );
...
async function toggleFollowUser() {
    ...
    const isFollowing =
        currentUserData.following &&
        currentUserData.following.includes(
            targetUid
        );
            ...
            item.innerHTML = `
                ...
                >
                    ...
                    <div class="user-meta">
                        ...
                        <span
                            class="uname"
                            style="font-weight:600; font-size:13.5px;"
                        >
                            ${u.username}${getVerifiedBadge(u.uid)}
                        </span>
...
async function toggleFollow(
    ...
    if (isFollowing) {
        ...
        currentUserData.following.push(
            targetUid
        );
...
async function loadSocialData() {
    ...
    if (mutuals.length === 0) {
        ...
        mutualContainer.innerHTML =
            "<p style='font-size:13px; color:var(--text-secondary);'>Aún no tienes amigos mutuos. Sigue a alguien que también te siga.</p>";

        if (noChatsTxt)
            noChatsTxt.style.display =
                "block";
    ...
    for (const mUid of mutuals) {
        ...
        mItem.innerHTML = `
            ...
            <div
                class="user-info-flex"
                onclick="viewUserProfile('${mUser.uid}')"
                style="cursor: pointer; flex: 1;"
            >
                ...
                <span
                    ...
                    style="font-weight:600; font-size:13.5px;"
                >
                    ${mUser.username}${getVerifiedBadge(mUser.uid)}
                </span>
            ...
            <button
                class="btn-secondary"
                style="width: auto; padding: 6px 12px;"
                onclick="openChat('${mUser.uid}', '${mUser.username}', '${mUser.avatar}')"
            >
                Chatear
            </button>
...
function openChat(
    ...
    if (chatInputWrapper)
        chatInputWrapper.classList.remove(
            'hidden'
        );
                    ...
                    if (
                        ...
                        db
                            ...
                            .then(() => {
                                ...
                                console.log(
                                    "Avatar actualizado y comprimido exitosamente."
                                );
...
function createPost() {
    ...
    if (
        ...
        return alert(
            "Escribe algo o adjunta una imagen para publicar."
        );
    ...
    card.innerHTML = `
        ...
        <div class="tweet-header">
            ...
            <img
                ...
                class="tweet-avatar"
                alt="Avatar"
                onclick="viewUserProfile('${data.uid}')"
            ...
            <div>
                ...
                <h4
                    ...
                    onclick="viewUserProfile('${data.uid}')"
                >
                    @${data.username}${getVerifiedBadge(data.uid)}
                </h4>
